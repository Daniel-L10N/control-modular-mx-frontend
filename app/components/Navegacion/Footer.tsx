    ...
    <footer className="bg-slate-900 border-t border-slate-800" aria-labelledby="footer-heading">
      ...
      <div className="mx-auto max-w-7xl py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
          ...
          <div className="space-y-8 xl:col-span-1">
            ...
            <div className="flex space-x-6">
              {socialIcons.map((item) => (
                <a key={item.name} href={item.href} className="text-slate-500 hover:text-indigo-400 transition-colors">
        ...
        <div className="mt-12 border-t border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center text-sm text-slate-500">
          <p>&copy; {new Date().getFullYear()} Control Modular MX. Todos los derechos reservados.</p>
          <p className="mt-4 md:mt-0">Ingeniería que revoluciona tu industria.</p>
        </div>
      </div>
    </footer>
  )
